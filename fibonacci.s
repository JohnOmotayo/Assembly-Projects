...
_start:
			LDR R0, =N		//R0 points to the address which fib we want
...
FIB: 		PUSH {R0 - R3} 		//push all the arguments on the stack so we don't lose information (higher order function)
			...
			BLE BASE			//check for negative flag
...
RETURN:  	STR R3, [SP, #16]	//store actual result on stack where OG argument was
			POP {R0 - R3}		//go back to caller with OG register contents
			BX LR
			
BASE:		MOV R3, #1			//moves content of 1 in the register
			B RETURN
...
N:			.word	10			//fib value to calculate
FIN:		.space	4   		//make space
